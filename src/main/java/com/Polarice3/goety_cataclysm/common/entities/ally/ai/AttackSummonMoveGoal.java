...
public class AttackSummonMoveGoal extends Goal {
    private final LLibrarySummon Boss_monster;
    private final boolean followingTargetEvenIfNotSeen;
    private Path path;
    private int delayCounter;
    protected final double moveSpeed;

    public AttackSummonMoveGoal(LLibrarySummon boss, boolean followingTargetEvenIfNotSeen, double moveSpeed) {
        ...
        if (target != null) {
            ...
            if (--this.delayCounter <= 0) {
                ...
                if (distSq > Math.pow(this.Boss_monster.getAttribute(Attributes.FOLLOW_RANGE).getValue(), 2.0)) {
                    if (!this.Boss_monster.isPathFinding() && !this.Boss_monster.getNavigation().moveTo(target, 1.0)) {
                        this.delayCounter += 5;
                    }
                } else {
