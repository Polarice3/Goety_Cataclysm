...
public class DeeplingBruteSpell extends SummonSpell {

    public int defaultSoulCost() {
        return GCSpellConfig.DeeplingBruteCost.get();
    }

    public int defaultCastDuration() {
        return GCSpellConfig.DeeplingBruteDuration.get();
    }

    public int SummonDownDuration() {
        return GCSpellConfig.DeeplingBruteSummonDown.get();
    }
    ...
    public int defaultSpellCooldown() {
        return GCSpellConfig.DeeplingBruteCoolDown.get();
    }
    ...
    public void commonResult(ServerLevel worldIn, LivingEntity caster){
        if (isShifting(caster)) {
            for (Entity entity : worldIn.getAllEntities()) {
                if (entity instanceof DeeplingBruteServant) {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                DeeplingBruteServant summonedentity = new DeeplingBruteServant(GCEntityType.DEEPLING_BRUTE.get(), worldIn);
