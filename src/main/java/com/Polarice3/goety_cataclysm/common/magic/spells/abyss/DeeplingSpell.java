...
public class DeeplingSpell extends SummonSpell {

    public int defaultSoulCost() {
        return GCSpellConfig.DeeplingCost.get();
    }

    public int defaultCastDuration() {
        return GCSpellConfig.DeeplingDuration.get();
    }

    public int SummonDownDuration() {
        return GCSpellConfig.DeeplingSummonDown.get();
    }
    ...
    public int defaultSpellCooldown() {
        return GCSpellConfig.DeeplingCoolDown.get();
    }
    ...
    public Predicate<LivingEntity> summonPredicate() {
        return livingEntity -> livingEntity instanceof DeeplingServant || livingEntity instanceof DeeplingAnglerServant;
    }
    ...
    public void commonResult(ServerLevel worldIn, LivingEntity caster){
        if (isShifting(caster)) {
            for (Entity entity : worldIn.getAllEntities()) {
                if (entity instanceof DeeplingServant || entity instanceof DeeplingAnglerServant) {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                AbstractDeeplingServant summonedentity = new DeeplingServant(GCEntityType.DEEPLING.get(), worldIn);
                if (worldIn.getRandom().nextFloat() <= 0.25F){
                    summonedentity = new DeeplingAnglerServant(GCEntityType.DEEPLING_ANGLER.get(), worldIn);
                }
