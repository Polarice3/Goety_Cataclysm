...
public class EliteDraugrSpell extends SummonSpell {

    public int defaultSoulCost() {
        return GCSpellConfig.EliteDraugrCost.get();
    }

    public int defaultCastDuration() {
        return GCSpellConfig.EliteDraugrDuration.get();
    }

    public int SummonDownDuration() {
        return GCSpellConfig.EliteDraugrSummonDown.get();
    }
    ...
    public int defaultSpellCooldown() {
        return GCSpellConfig.EliteDraugrCoolDown.get();
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            int i = 1;
            if (rightStaff(staff)){
                i = 2 + worldIn.random.nextInt(4);
            } else if (typeStaff(staff, SpellType.FROST)){
                i = 2;
            }
            for (int i1 = 0; i1 < i; ++i1) {
                EliteDraugrServant summonedentity = new EliteDraugrServant(GCEntityType.ELITE_DRAUGR_SERVANT.get(), worldIn);
