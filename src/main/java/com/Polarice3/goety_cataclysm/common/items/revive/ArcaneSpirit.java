...
public class ArcaneSpirit extends ReviveServantItem {
    public ArcaneSpirit() {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null){
            ...
            entity = new WadjetServant(GCEntityType.WADJET.get(), level);
            IOwned owned = (IOwned) entity;
            owned.setTrueOwner(player);
        }
        if (entity instanceof WadjetServant servant) {
            boolean flag = target instanceof KoboletonServant || target instanceof Koboleton_Entity;
            if (flag && servant.getTrueOwner() == player && RitualRequirements.canSummon(level, player, GCEntityType.WADJET.get())) {
                ...
                if (level.addFreshEntity(servant)) {
                    ...
                    servant.playSound(CataclysmSounds.WADJET_AMBIENT.get(), 2.0F, 0.5F);
                    target.discard();
                    player.swing(hand);
