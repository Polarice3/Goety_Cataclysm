...
public class IgnitedHelm extends ReviveServantItem {
    public IgnitedHelm() {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null){
            ...
            entity = new IgnitedRevenantServant(GCEntityType.IGNITED_REVENANT.get(), level);
            IOwned owned = (IOwned) entity;
            owned.setTrueOwner(player);
        }
        if (entity instanceof IgnitedRevenantServant servant) {
            boolean flag = target instanceof Wildfire;
            if (GCMobsConfig.BlazeIgnitedHelm.get()){
                flag = target instanceof Wildfire || target instanceof Blaze || target instanceof BlazeServant;
            }
            if (flag && servant.getTrueOwner() == player && RitualRequirements.canSummon(level, player, GCEntityType.IGNITED_REVENANT.get())) {
