...
public class IgnisAbyssFireballRenderer extends EntityRenderer<IgnisAbyssFireball> {
	...
	public IgnisAbyssFireballRenderer(EntityRendererProvider.Context manager) {
		super(manager);
		this.model = new Ignis_Fireball_Model();
	}
	
	@Override
	protected int getBlockLightLevel(IgnisAbyssFireball entity, BlockPos pos) {
		return 15;
	}

	@Override
	public void render(IgnisAbyssFireball entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(IgnisAbyssFireball entity) {
		return  IGNIS_FIRE_BALL;
	}

	private void renderTrail(IgnisAbyssFireball entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailR, float trailG, float trailB, float trailA, int packedLightIn) {
		int sampleSize = 10;
