...
public class AbyssBlastRenderer extends EntityRenderer<AbyssBlast> {
    ...
    public AbyssBlastRenderer(EntityRendererProvider.Context mgr) {
        super(mgr);
    }

    @Override
    public ResourceLocation getTextureLocation(AbyssBlast entity) {
        return TEXTURE;
    }


    public boolean shouldRender(AbyssBlast solarBeam, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }

    @Override
    public void render(AbyssBlast solarBeam, float entityYaw, float delta, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        clearerView = solarBeam.getOwner() instanceof Player && Minecraft.getInstance().player == solarBeam.getOwner() && Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON;
